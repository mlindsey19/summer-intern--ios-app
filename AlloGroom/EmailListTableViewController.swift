...
//  AlloGroom
//
//  Created by Daniel Reller on 8/16/18.
...
class EmailListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? contactTableViewCell else {
            fatalError("The dequed cell was not an instance of conactTableViewCell")
        }

    let contact = contactsArray[indexPath.row]
        let lastname = contact.lastName ?? ""
        cell.nameLabel?.text = "\(contact.firstName) \(lastname)"
